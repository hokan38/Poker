...
- **Filter / subset check — before quoting any range-aggregate.** The レンジ比較 / フィルター view and hovered subsets show a *tiny* combo count; their aggregates (range equity / EV / EQR, and the range-level action %) are **NOT** whole-range. Before quoting a range-aggregate, confirm the **combo count** is plausible for the node (an SRP range is hundreds of combos). Only take whole-range % from the unfiltered 戦略 action panel; cross-check the same node across frames (the count must match). A **hand's own cell %** (e.g. a pinned `88 → Check 99.9%`) and **per-combo splits** are always safe — they don't move with a range filter.

...
| ターン | + Q | 6.5 | Check | **Bet 4.4**（≈68% pot） | CO **Call** |
| リバー | + K | 15.3 | Check | **Check**（88でチェックバック） | ショウダウン |
...
あなたの手は **8♠8♣＝スペード（フラッシュドロー）ブロッカー持ち**なので、**Bet 4.4 は正解の側**。
狙いは **薄いバリュー＋Protection（相手のオーバーカードやスペードドローのEQ否定）＋ポット形成**。
...
リバー K で盤面は **J-9-5-Q-K**。**T があれば 9-T-J-Q-K、A があれば T-J-Q-K-A のストレートが完成する**、
非常にストレートの入りやすいカードです。CO がチェック。正解は **チェックバック**。

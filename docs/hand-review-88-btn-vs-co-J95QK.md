...
- BTNは**レンジ優位**（約半分をベットできるスポット）で、ややバリュー寄りに打てる。相手は理論上 **MDF ≈ 60%**（＝6.5/10.9）を続行する必要がある。

![turn strategy: Bet 4.4 51.7% / Check 48.3% on J95Q](images/hand-88-btn-co/03-turn-strategy.png)

88単体は**混合**、しかも**スート依存**でした（ブロッカー効果）。

- 8**♠** を持つ **8♠8♣ → Bet 41.9% / Check 58.1%**（8♠8♥ は 49.2%）
- スペードを持たない **8♥8♦ → Bet 0.7% / Check 99.3%**
...
> ※このパネルは**レンジ絞り込み表示**（コンボ数が少ないサブセット）。各88コンボの選択頻度は正確だが、パネル上のレンジEQ%（59.4%等）はレンジ全体の値ではないので採用していない。
...
- リバーのベットは **ポット100%のポラー・サイズ**（理論上バリュー:ブラフ = (b+p):b = **2:1**、相手のMDF = p/(b+p) = **50%**）。打てるのは**ナッツ級バリュー＋ブラフだけ**。
- そして **88 は Bet 0.1% / Check 99.9%＝ほぼ純チェック**（セル値なので正確）。

![river strategy: polar pot-size bet; 88 checks 99.9%](images/hand-88-btn-co/05-river-strategy.png)

なぜ、ポラーに打てるスポットで 88 は打たないのか。理由は4つです。
...
[切り離しと押し上げ](./amu-goole-poker-notes.md#ポストフロップとriver) が効く局面です。要点は概念で押さえます。

- **EQ（生の勝率）だけでは戦略は決まらない。** リバーでは相手のトラッシュを**切り離し**、拮抗（call）レンジに対して測り直す。
- **EQ ≠ EV ≠ EQR。** 生EQがほぼ互角でも、**IP＋ポラライズ**なら実現率(EQR)は100%を超え、EVは打ち手側に傾きうる。
- だから **「EQがあるからベット」ではなく「打てるハンド（ナッツ級＋ブラフ）だけがベット、中間の88はチェック」** に落ち着く。

> ⚠️ 補足（正確性）: 下のGTO Wizardの「EQ/EV/EQR」パネルは**レンジ絞り込み表示**（BTNのコンボ数が極端に少ないサブセット）だったため、**レンジ全体のEQ/EV/EQRの具体数値としては採用していません**。各ハンドの選択頻度（88=Check 99.9% 等）はセル値なので正確です。

![river EQ/EV/EQR panel (filtered subset view)](images/hand-88-btn-co/06-river-eq-ev-eqr.png)
